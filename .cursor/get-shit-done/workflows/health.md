...
```bash
node "G:/ONGOING PROJECTS/Digital_product-auto_poster/.cursor/get-shit-done/bin/gsd-tools.cjs" validate health $REPAIR_FLAG
```
...
```bash
node "G:/ONGOING PROJECTS/Digital_product-auto_poster/.cursor/get-shit-done/bin/gsd-tools.cjs" validate health
```
...
TASKS_DIR="G:/ONGOING PROJECTS/Digital_product-auto_poster/.cursor/tasks"
if [ -d "$TASKS_DIR" ]; then
  STALE_COUNT=$( (find "$TASKS_DIR" -maxdepth 1 -type d -mtime +1 2>/dev/null || true) | wc -l )
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in G:/ONGOING PROJECTS/Digital_product-auto_poster/.cursor/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf G:/ONGOING PROJECTS/Digital_product-auto_poster/.cursor/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
